    ...
    while (*text) {
        if (*text >= 'A' && *text <= 'Z') {
            ...
            for (int i = 0; i < 7; i++) {
                ...
                while (*row) {
                    if (*row == '0') {
                        draw_rect(current_x, y, half_size, half_size, color);
                    }
                    current_x += size;
                    row++;
                }
            ...
            const char **letter = letters[26]; // '/' character
            float current_x = x;
        ...
        x += size * 6.0f; // Advance the cursor position
        y = original_y; // Reset y position for the next line of text
        text++;
    }
}
