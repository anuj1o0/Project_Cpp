...
simulate_game(Input* input, float dt) {
	...
	if (current_gamemode == GM_GAMEPLAY) {
		...
		if (!enemy_is_ai) {
			...
			else if (player_1_score >= 10 || player_2_score >= 10) {
		...
		if (player_1_score > player_2_score) {
			draw_text("PLAYER A WON", -40, 0, 1, 0xff0000);
		}
		else if (player_1_score < player_2_score) {
			draw_text("PLAYER B WON", -40, 0, 1, 0xff0000);
		}
		else {
			draw_text("IT IS DRAW", -40, 0, 1, 0xff0000);
		}

		draw_text("PRESS ENTER FOR NEW GAME", -70, -20, 1, 0xffffff);
		...
		draw_text("   PONG GAME   ", -83, 40, 2, 0xffffff);
		draw_text("PLAYER A   VS   PLAYER B", -53, 10, .75, 0xffffff);
		//draw_text("DISIGNED BY AREA 51", -73, -20, 1.22, 0xffffff);
